...
auto fiska::x86::fe::Parser::parse_x86_instr_expr() -> X86InstrExpr* {
    ...
    auto x86_instr = new (ctx_) X86InstrExpr;
...
auto fiska::x86::fe::Parser::parse_expr(i8 prec) -> Expr* {
    ...
    case TK::BitWidth: {
        ...
        if (match(TK::At, TK::BitWidth, TK::LBracket)) {
            ...
            auto mem = new (ctx_) MemRefLitExpr;
        ...
        if (match(TK::At, TK::BitWidth)) {
            ...
            auto moffs = new (ctx_) MoffsLitExpr;
    ...
    case TK::Ident: {
        consume(TK::Ident);
        lhs = new (ctx_) LabelExpr(peek_tok_str(-1));
        break;
    }
    case TK::Num: {
        consume(TK::Num);
        lhs = new (ctx_) IntLitExpr(Parser::parse_i64(peek_tok_str(-1))); 
        break;
    }
    case TK::Plus:
    case TK::Minus: {
        ...
        lhs = new (ctx_) IntLitExpr(result);
        break;
    }
    ...
    while (Parser::infix_prec_of_tok_kind(peek_tok_kind()) > prec) {
        ...
        lhs = new (ctx_) BinaryOpExpr(binop, binop_lhs, binop_rhs);
    }
